		...
		public function updateContentElements($data)
		{
			...
			foreach ($data as $result) {
				$connection->update(
					$this->table,
					[
						'colPos' => (empty($result['sameCid'])) ? $result['columnid'] : $result['sameCid'],
						'tx_container_parent' => $result['gridUid'],
			...
			foreach ($elementsArray as $key => $element) {
				if ($elementsArray[$key]['active'] == 1) {
					$elementsArray[$key]['contentelements'] = $queryBuilder
						...
						->where(
							$queryBuilder->expr()->like('CType', '"%gridelements_pi%"'),
							$queryBuilder->expr()->like('tx_gridelements_backend_layout', '"%'.$key.'%"')
						)
						->execute()
						->fetchAll(\Doctrine\DBAL\FetchMode::ASSOCIATIVE);
				} else {
				...
				foreach ($results as $key2 => $elements) {
					if ($key2 == 'contentelements') {
						foreach ($results[$key2] as $element) {
							...
							$queryBuilder = $connection->createQueryBuilder();
							$queryBuilder->getRestrictions()->removeAll()->add(GeneralUtility::makeInstance(DeletedRestriction::class));
							$contentElementResults[$key]['elements'][$element['uid']] = $queryBuilder
								->select('*')
								->from($this->table)
								->where(
								...
								foreach ($elements as $element) {
									if ($element['tx_gridelements_columns'] == $key2) {
										...
										$connection->update(
											$this->table,
											[
												'colPos' => (empty($column['sameCid'])) ? $column['columnid'] : $column['sameCid'],
												'tx_container_parent' => $key3,
												'tx_gridelements_container' => 0,
												'tx_gridelements_columns' => 0
											],
											[
